...
ImuProcess::ImuProcess(const rclcpp::Node::SharedPtr node) : node_(node), b_first_frame_(true), imu_need_init_(true), gravity_align_(false) {
...
void ImuProcess::Reset() {
  ...
  RCLCPP_WARN(node_->get_logger(), "ImuProcess reset");
  mean_acc       = V3D(0, 0, 0.0);
  mean_gyr       = V3D(0, 0, 0);
  imu_need_init_ = true;
  init_iter_num  = 1;
...
void ImuProcess::IMU_init(const MeasureGroup &meas, int &N) {
  ...
  RCLCPP_INFO(node_->get_logger(), "Initializing: %.1f %%", double(N) / MAX_INI_COUNT * 100);
  V3D cur_acc, cur_gyr;
  ...
  if (imu_en) {
    ...
    if (imu_need_init_) {
      ...
      if (init_iter_num > MAX_INI_COUNT) {
        RCLCPP_INFO(node_->get_logger(), "Initializing: %.1f %%", 100.0);
